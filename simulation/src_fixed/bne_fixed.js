...
var int_pos, diag_int, diag_bed, rhog_int, rhog_bed, intnum, numg2, geng, r, v, m, D, m_total;
...
function setElementsLayout() {
	...
	tout(taConsoleH, "----- Realtime Output -----");
...
function loadParameters() {
	...
	lines += "RHOG_INT 700 800 900 1000 1100 1200 1300 1400\n";    	// Intruder density range   kg/m3
	lines += "RHOG_BED 700 800 900 1000 1100 1200 1300 1400\n";    	// Bed density range        kg/m3
	lines += "NUMG_BED 150\n";          // Number of bed
...
function readParameters() {
	...
	// for random, we use below
    //N_exp = getValue(lines,"NEXP");

    N_rhog_int = getNumArray(lines, "RHOG_INT")-1;
    N_rhog_bed = getNumArray(lines, "RHOG_BED")-1;
    N_exp = N_rhog_bed*N_rhog_int;

	var k = 0
    for(var i=0; i<N_rhog_bed; i++){
        for(var j=0; j<N_rhog_int; j++){
            rhog_int_list[k] = getValueArray(lines, "RHOG_INT",j);
            rhog_bed_list[k] = getValueArray(lines, "RHOG_BED",i);
            k++;
        }
    }
...
function initParams(){
	...
	rhog_int = rhog_int_list[nexp] 
    rhog_bed = rhog_bed_list[nexp]
	numg	 = numg_bed + 1;
...
function simulate() {
	...
	if (t > tdelay) {
		...
		vibOn = true;
		if(t_console == 100){
			tout(taConsole,t.toFixed(2)+'\t'+getContactopy()+'\t'+getRise().toFixed(2)+'\t'+r[0].y.toFixed(2)+'\t'+potential_int().toFixed(3)+'\t'+potential_bed().toFixed(3)+'\n');
			t_console = 0;
		}
		t_console++;
	}
...
function potential_bed() {
	var U_bed = 0;
	for(var i = 1; i<numg; i=i+1){
		U_bed = U_bed + m[i]*gacc*r[i].z;
	}
	return U_bed;
}

function potential_int(){
	var U_int = m[0]*gacc*r[0].z;
